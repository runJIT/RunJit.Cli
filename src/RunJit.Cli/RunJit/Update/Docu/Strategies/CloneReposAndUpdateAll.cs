                                                 ...
                                                 SlackSettings slackSettings) : IUpdateGlobalJsonServiceStrategy
    {
        ...
        public async Task HandleAsync(UpdateGlobalJsonParameters parameters)
        {
            ...
            foreach (var repo in repos)
            {
                ...
                var branchName = "quality/update-docu";
                ...
                var pullRequestInfo = await awsCodeCommit.CreatePullRequestAsync("Update docu.",
                                                           "Update docu.", qualityCheckBackendBuildsPackages).ConfigureAwait(false);
                ...
                await slackApiClient.Chat.PostMessage(new Message
                                                      {
                                                          Text = $"{moduleName}: Update docu{Environment.NewLine}<{pullRequestInfo.AbsoluteUrl}>",
